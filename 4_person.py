...
def main():
    video_path = input("Enter path to main video file: ")
    ...
    # Ask the user for idle video (ideal state) file.
    idle_path = input("Enter path to idle video file for ideal state (or press ENTER to skip): ")
    idle_cap = None
    if idle_path:
        if not os.path.exists(idle_path):
            print(f"Error: Idle video file '{idle_path}' not found.")
        else:
            idle_cap = cv2.VideoCapture(idle_path)

    cap = cv2.VideoCapture(video_path)
    if not cap.isOpened():
        print("Error opening main video file!")
        return
    ...
    print(f"Main Video: {width}x{height}, {fps} FPS, {frame_count} frames")
    ...
    while running:
        ...
        if not paused:
            ...
            # Adjust the main video frame index if behind
            ...
            # Determine the current audio MAR if available
            current_audio_mar = None
            if audio_mars is not None and frame_idx < len(audio_mars):
                current_audio_mar = audio_mars[frame_idx]

            # Decide whether to use idle video or main video frame
            if current_audio_mar is not None and current_audio_mar < 0.21 and idle_cap is not None:
                ret_idle, frame = idle_cap.read()
                if not ret_idle:
                    idle_cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
                    ret_idle, frame = idle_cap.read()
                # In idle state, we simply use the idle frame without lip sync processing.
                processed_frame = frame.copy()
            else:
                ret, frame = cap.read()
                if not ret:
                    print("End of main video reached")
