...
class VideoMouthExtractor:
    def __init__(self,  viseme_mapper=None):
        # Initialize dlib's detector and predictor
        self.detector = dlib.get_frontal_face_detector()
        self.viseme_mapper = viseme_mapper
        if self.viseme_mapper is None:
    ...
    def extract_phoneme_mouth_images(self, video_path, output_folder="mouth_images", json_file="phoneme_map.json"):
        """
        Extract mouth images from video and create a direct mapping between phonemes and image files.
        This creates a comprehensive JSON file that maps each phoneme to its best matching mouth image.
        """
        import json

        print(f"Extracting phoneme mouth images from: {video_path}")
        ...
        # Initialize direct phoneme mapping
        phoneme_map = {}

        # Define all phonemes we want to capture
        all_phonemes = [
            # Vowels
        ...
        while True:
            ...
            try:
                ...
                if len(faces) > 0:
                    ...
                    # Save the mouth image with viseme label
        ...
        # Create direct phoneme-to-image mapping
        # This maps each phoneme directly to the best image for its corresponding viseme
        for phoneme in all_phonemes:
            # Get the viseme for this phoneme using the viseme mapper
            viseme = self.viseme_mapper.phoneme_to_viseme.get(phoneme, "REST")

            # If we have a best example for this viseme, use it
            if viseme in best_examples:
                phoneme_map[phoneme] = {
                    'viseme': viseme,
                    'image': best_examples[viseme]['filename'],
                    'mar': best_examples[viseme]['mar']
                }
            else:
                # Fallback to REST viseme if the specific viseme wasn't found
                if "REST" in best_examples:
                    phoneme_map[phoneme] = {
                        'viseme': "REST",
                        'image': best_examples["REST"]['filename'],
                        'mar': best_examples["REST"]['mar']
                    }
        ...
        print(f"Created phoneme mapping in {json_file}")
        print(f"Mapped {len(phoneme_map)} phonemes to {len(best_examples)} visemes")
...
class RealisticLipSyncAnimator:
    def __init__(self, width=800, height=600):
        ...
        # Video processing
        self.video_extractor = VideoMouthExtractor(self.viseme_mapper)
        ...
        self.idle_timer = 0
    def animate_with_direct_phoneme_mapping(self, audio_file, speaking_video_path, silence_video_path,
                                            json_mapping_file="phoneme_map.json", script_text=None):
        """
        Enhanced animation that uses direct phoneme-to-image mapping from the JSON file.
        If script_text is None, it will attempt to generate it from the audio file.
        """
        import json
        ...
        try:
            with open(json_mapping_file, 'r') as f:
                mapping = json.load(f)
            print(f"Loaded phoneme mapping from {json_mapping_file}")

            # Verify the mapping has the required structure
            if 'phoneme_map' not in mapping:
                print("Error: Invalid mapping file format. Missing 'phoneme_map' key.")
                return
        except Exception as e:
            print(f"Error loading mapping file: {e}")
            return
        ...
        # Generate phoneme sequence from script text if provided
        if script_text:
            # Get audio duration
            import librosa
            y, sr = librosa.load(audio_file, sr=None)
            ...
            # Convert to phoneme sequence with image filenames
            phoneme_sequence = []
            for phoneme, start, end in phoneme_timings:
                # Get the image for this phoneme from the mapping
                image_filename = None
                if phoneme in mapping['phoneme_map']:
                    image_filename = mapping['phoneme_map'][phoneme]['image']
                else:
                    # Fallback to REST if phoneme not found
                    if 'SIL' in mapping['phoneme_map']:
                        image_filename = mapping['phoneme_map']['SIL']['image']

                # Add to sequence
                phoneme_sequence.append((phoneme, image_filename, start, end))

            print(f"Generated phoneme sequence with {len(phoneme_sequence)} segments")
        else:
        ...
        print("Ready to animate with direct phoneme mapping. Press SPACE to start playback.")
        ...
        while running:
            ...
            if start_time is not None and not paused:
                ...
                # Get current phoneme and image filename from sequence
                current_phoneme = "SIL"
                current_image = None

                if phoneme_sequence:
                    for phoneme, image_filename, start, end in phoneme_sequence:
                        if start <= current_time < end:
                ...
                # Determine whether to use speaking or silence frame
                use_silence = (current_phoneme in ["SIL", "SP"])
                ...
                if base_frame is not None:
                    ...
                    # If we have a mapped image for this phoneme, use it
                    if current_image:
                        # Load the mouth image
                        mouth_img_path = os.path.join(os.path.dirname(json_mapping_file), current_image)
                        if os.path.exists(mouth_img_path):
                    ...
                    phoneme_text = font.render(f'Phoneme: {current_phoneme}', True, (255, 255, 255))
                    self.screen.blit(phoneme_text, (10, self.height - 50))
    ...
    def generate_script_from_audio(self, audio_file):
        ...
        try:
            ...
            # Check if the audio file is in a format that SpeechRecognition can handle directly
            # If not, convert it using librosa
            try:
                with sr.AudioFile(audio_file) as source:
                    # Read the audio data
                    audio_data = r.record(source)
            except Exception as e:
                print(f"Error opening audio file directly: {e}")
                print("Attempting to convert using librosa...")
            ...
            print("Install them with: pip install SpeechRecognition librosa soundfile")
            return None
        except Exception as e:
...
def main():
    ...
    print("2. Extract mouth images with direct phoneme mapping")
    print("3. Run enhanced lip sync with direct phoneme mapping")
    print("4. Generate script from audio file")
    ...
    if choice == "1":
        ...
        # Extract mouth images with direct phoneme mapping
        ...
        json_file = input("Enter JSON mapping file name (default: phoneme_map.json): ") or "phoneme_map.json"

        if not os.path.exists(video_path):
            print(f"Error: Video file {video_path} not found.")
            return

        animator.video_extractor.extract_phoneme_mouth_images(video_path, output_folder, json_file)
        print(f"Extracted mouth images to {output_folder} and created phoneme mapping in {json_file}")

    elif choice == "3":
        # Run enhanced lip sync with direct phoneme mapping
        audio_file = input("Enter path to audio file (default: sample_audio.wav): ") or "sample_audio.wav"
        speaking_video = input("Enter path to speaking video file (default: ./speaking.mp4): ") or "./speaking.mp4"
        silence_video = input("Enter path to silence video file (default: ./silence.mp4): ") or "./silence.mp4"
        json_file = input("Enter JSON mapping file (default: phoneme_map.json): ") or "phoneme_map.json"

        # Ask if user wants to generate script from audio
        use_audio_script = input("Generate script from audio? (y/n, default: n): ").lower() == 'y'
        script_text = None

        if not use_audio_script:
            script_text = input("Enter script text (or press Enter to skip): ")
        ...
        if not os.path.exists(json_file):
            print(f"Warning: JSON mapping file {json_file} not found.")
            create_mapping = input("Do you want to create the mapping first? (y/n): ")
            if create_mapping.lower() == 'y':
                animator.video_extractor.extract_phoneme_mouth_images(speaking_video, "mouth_images", json_file)
            else:
                print("Cannot proceed without mapping file.")
                return

        animator.animate_with_direct_phoneme_mapping(audio_file, speaking_video, silence_video, json_file, script_text)

    elif choice == "4":
        # Generate script from audio
        ...
        script = animator.generate_script_from_audio(audio_file)
        if script:
            print(f"Generated script: '{script}'")
            save_script = input("Save script to file? (y/n): ")
            if save_script.lower() == 'y':
                script_file = input("Enter filename (default: script.txt): ") or "script.txt"
