...
class RealisticLipSyncAnimator:
    ...
    def load_mouth_images(self, mouth_images_dir="mouth_images"):
        """Load pre-extracted mouth images for each viseme."""
        self.mouth_images = {}

        if not os.path.exists(mouth_images_dir):
            print(f"Error: Mouth images directory {mouth_images_dir} not found")
            return False

        # Load images for each viseme
        for viseme in ["REST", "I", "A", "O", "Wide"]:
            self.mouth_images[viseme] = []

            # Look for images with this viseme prefix
            for filename in os.listdir(mouth_images_dir):
                if filename.startswith(f"{viseme}_"):
                    filepath = os.path.join(mouth_images_dir, filename)
                    image = cv2.imread(filepath)
                    if image is not None:
                        self.mouth_images[viseme].append(image)

            if not self.mouth_images[viseme]:
                print(f"Warning: No images found for viseme {viseme}")
            else:
                print(f"Loaded {len(self.mouth_images[viseme])} images for viseme {viseme}")
    ...
    # Add this method to the RealisticLipSyncAnimator class

    def load_viseme_images(self, viseme_images_dir="viseme_images"):
        """
        Load pre-extracted viseme images for animation.

        Args:
            viseme_images_dir: Directory containing the extracted viseme images
        ...
        if not os.path.exists(viseme_images_dir):
            print(f"Error: Viseme images directory {viseme_images_dir} not found")
            return False

        # Try to load the mapping file
        mapping_file = os.path.join(viseme_images_dir, "viseme_mapping.json")
        if os.path.exists(mapping_file):
            try:
                with open(mapping_file, 'r') as f:
                    viseme_to_image_paths = json.load(f)

                # Load each image from the paths
                for viseme, image_paths in viseme_to_image_paths.items():
                    self.viseme_images[viseme] = []
                    for path in image_paths:
                        if os.path.exists(path):
                            img = cv2.imread(path)
                            if img is not None:
                                self.viseme_images[viseme].append(img)

                    print(f"Loaded {len(self.viseme_images[viseme])} images for viseme {viseme}")

                return True
            except Exception as e:
                print(f"Error loading viseme mapping: {e}")

        # If no mapping file or error occurred, try to load images directly from directory
        print("No mapping file found. Loading images directly from directory...")

        # Get all viseme categories from our viseme map
        all_visemes = list(self.viseme_map.keys())

        # Initialize empty lists for each viseme
        for viseme in all_visemes:
            self.viseme_images[viseme] = []

        # Load images from directory
        for filename in os.listdir(viseme_images_dir):
            if filename.endswith(('.png', '.jpg', '.jpeg')):
                # Parse viseme from filename (format: VISEME_XXX_mar_Y.YY.png)
                parts = filename.split('_')
                if len(parts) >= 2:
                    viseme = parts[0]
                    if viseme in all_visemes:
                        filepath = os.path.join(viseme_images_dir, filename)
        ...
        # Print summary
        total_images = 0
        for viseme, images in self.viseme_images.items():
            print(f"Loaded {len(images)} images for viseme {viseme}")
            total_images += len(images)

        print(f"Loaded {total_images} viseme images total")
        return total_images > 0

    def adjust_mouth_image(self, mouth_img, viseme_name):
        """
        Apply manual adjustments to mouth images based on viseme type
        """
        # Make a copy to avoid modifying the original
        adjusted_img = mouth_img.copy()

        # Apply viseme-specific adjustments
        if viseme_name == "A":
            # For "A" viseme, make the mouth more open
            # Stretch vertically
            adjusted_img = cv2.resize(adjusted_img,
                                      (adjusted_img.shape[1],
                                       int(adjusted_img.shape[0] * 1.3)))
        elif viseme_name == "O":
            # For "O" viseme, make the mouth more round
            # Apply morphological operations to enhance roundness
            kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
            adjusted_img = cv2.morphologyEx(adjusted_img, cv2.MORPH_CLOSE, kernel)
        elif viseme_name == "REST":
            # For REST, slightly compress the mouth
            adjusted_img = cv2.resize(adjusted_img,
                                      (adjusted_img.shape[1],
                                       int(adjusted_img.shape[0] * 0.9)))
    ...
    def process_frame_with_viseme(self, frame, viseme_name, viseme_params):
        ...
        # If we have pre-extracted viseme images for this viseme
        if hasattr(self, 'viseme_images') and viseme_name in self.viseme_images and self.viseme_images[viseme_name]:
            # Get a random mouth image for this viseme
            random_idx = np.random.randint(0, len(self.viseme_images[viseme_name]))
            mouth_img = self.viseme_images[viseme_name][random_idx]
            ...
            if len(faces) > 0:
                ...
                # Calculate mouth aspect ratio
                mar_value = self.video_extractor.mouth_aspect_ratio(mouth_points)
...
def main():
    ...
    print("2. Extract viseme images from video")
    choice = input("Enter your choice (1 or 2): ")

    if choice == "2":
        # Extract viseme images
        from animator_with_static_lips_helper import extract_viseme_images
        ...
        extract_viseme_images(video_path, output_dir, max_per_viseme)
        print("Extraction complete. You can now run the lip sync animation.")
        return

    # Run lip sync animation
